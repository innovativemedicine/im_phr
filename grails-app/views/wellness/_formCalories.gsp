        ...
        <ul class="block-text-18">
            ...
            <g:else>
				<g:each in="${UserCaloriesInstanceList }" var="data">
				    <li>
				        <span class="value-block">${data.amount } calories</span> 
        ...
        <g:form action="saveCalories" class="form-hidden" >
	        <div id="calories_add" class="add-section" style="display: none;">
	            ...
	            <div class="fieldcontain ${hasErrors(bean: userCaloriesInstance, field: 'user', 'error')} required hidden">
	                ...
	                <g:select id="user" name="user.id" from="${phr.Login.findByUserName(session.user.userName)}" optionKey="id" required="" value="${userCaloriesInstance?.user?.id}" class="many-to-one"/>
	            </div>
