			...
			<div class="row-fluid">
				<div class="block span3">
					...
					<div class="block-text">
						...
						<img src="${createLink(controller:'MyProfile', action:'showPayload', id:"${session.user.id}")}" width='200'
							class="profile-img" />
						<g:fieldValue bean="${fileInstance}" field="fileName" />
						<br>

						<g:each in="${UserProfileInstanceList }" var="data">
							<span> ${data.first_name } <g:if test="${data.middle_name != null}">
									${data.middle_name }
								</g:if> ${data.last_name }
							</span>
							<br />
							...
							<g:if test="${data.home_phone != null }">
								<span>Home #: ${data.home_phone }</span>
								<br />
							</g:if>
							<g:if test="${data.cell_phone != null }">
								<span>Cell #: ${data.cell_phone }</span>
								<br />
							</g:if>
							...
							<span>Birthday: <g:formatDate format="yyyy-MM-dd" date="${data.birthday}"></g:formatDate></span>
							<br />
							<g:if test="${data.blood_type != null }">
								<span>Blood Type: ${data.blood_type }</span>
								<br />
							</g:if>
							<g:if test="${data.height != null }">
								<span>Height: ${data.height } inches
								</span>
								<br />
							</g:if>
							<g:if test="${data.marital_status != null }">
								<span>Marital Status: ${data.marital_status }</span>
								<br />
							</g:if>
							<g:if test="${data.primary_language != null }">
								<span>Primary Language: ${data.primary_language }</span>
								<br />
							</g:if>
							<g:if test="${data.weight != null }">
								<span>Weight: ${data.weight } pounds
								</span>
								<br />
							</g:if>
					...
					<div class="block-text">
						...
						<g:if test="${UserCaloriesInstanceList.size != 0 }">
							<g:each in="${UserCaloriesInstanceList }" var="data">
								<span><strong>Most recent calories:</strong> ${data.amount } calories [${data.date }]</span>
								<br />
							</g:each>
						</g:if>
						<g:if test="${UserCarbohydratesInstanceList.size != 0 }">
							<g:each in="${UserCarbohydratesInstanceList }" var="data">
								<span><strong>Most recent carbohydrates:</strong> ${data.amount } grams [${data.date }]</span>
								<br />
							</g:each>
						</g:if>
						<g:if test="${UserCholestrolInstanceList.size != 0 }">
							<g:each in="${UserCholestrolInstanceList }" var="data">
								<span><strong>Most recent cholestrol:</strong> ${data.amount } mg/dl [${data.date }]</span>
								<br />
							</g:each>
						</g:if>
						<g:if test="${UserBloodPressureInstanceList.size != 0 }">
							<g:each in="${UserBloodPressureInstanceList }" var="data">
								<span><strong>Most recent blood pressure:</strong> ${data.systolic }/${data.diastolic } mm Hg [${data.date }]</span>
								<br />
							</g:each>
						</g:if>
						<g:if test="${UserBmiInstanceList.size != 0 }">
							<g:each in="${UserBmiInstanceList }" var="data">
								<span><strong>Most recent height:</strong> ${data.height } inches [${data.date }]</span>
								<br />
								<span><strong>Most recent weight:</strong> ${data.weight } lbs [${data.date }]</span>
								<br />
								<span><strong>Most recent BMI:</strong> ${data.bmi } kg/m2 [${data.date }]</span>
								<br />
							</g:each>
						</g:if>
