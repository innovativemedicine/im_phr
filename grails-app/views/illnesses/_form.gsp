...
<div class="fieldcontain ${hasErrors(bean: userIllnessesInstance, field: 'user', 'error')} required hidden">
    ...
    <g:select id="user" name="user.id" from="${phr.Login.findByUserName(session.user.userName)}" optionKey="id" required="" value="${userIllnessesInstance?.user?.id}" class="many-to-one"/>
</div>
