...
<div class="fieldcontain ${hasErrors(bean: userAllergiesInstance, field: 'user', 'error')} required">
    ...
    <g:select id="user" name="user.id" from="${im.Login.findByFirstName(session.user.firstName)}" optionKey="id" required="" value="${userAllergiesInstance?.user?.id}" class="many-to-one"/>
</div>
