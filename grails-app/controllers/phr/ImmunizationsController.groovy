...
class ImmunizationsController {
	...
	def save() {
		...
		if (!UserImmunizationsInstance.save(flush: true)) {
			flash.message = message(code: 'Error saving the entry. Please ensure the values are correct.', args: [
				message(code: 'UserImmunizations.label', default: 'UserImmunizations')
			])
			redirect(action: "create", params: params)
			return
		}

		flash.message = message(code: 'Immunization \"' + UserImmunizationsInstance.name + '\" added successfully', args: [
			message(code: 'UserImmunizations.label', default: 'UserImmunizations'),
			UserImmunizationsInstance.id
		])
	...
	def update(Long id, Long version) {
		...
		flash.message = message(code: 'Immunization \"' + UserImmunizationsInstance.name + '\" updated successfully', args: [
			message(code: 'UserImmunizations.label', default: 'UserImmunizations'),
			UserImmunizationsInstance.id
		])
	...
	def delete(Long id) {
		...
		try {
			...
			flash.message = message(code: 'Immunization \"' + UserImmunizationsInstance.name + '\" deleted successfully', args: [
				message(code: 'UserImmunizations.label', default: 'UserImmunizations'),
				UserImmunizationsInstance.id
			])
			redirect(controller: "healthInformation", action: "information")
