...
class AllergiesController {
	...
	def save() {
		...
		if (!UserAllergiesInstance.save(flush: true)) {
			flash.message = message(code: 'Error saving the entry. Please ensure the values are correct.', args: [
				message(code: 'UserAllergies.label', default: 'UserAllergies')
			])
			redirect(action: "create", params: params)
			return
		}

		flash.message = message(code: 'Allergy \"' + UserAllergiesInstance.name + '\" added successfully', args: [
			message(code: 'UserAllergies.label', default: 'UserAllergies'),
			UserAllergiesInstance.id
		])

		redirect(controller: "conditions", action: "conditions")
	}
	...
	def edit(Long id) {
		...
		if (!UserAllergiesInstance) {
			flash.message = message(code: 'Could not find the specific entry. Please try again.', args: [
				message(code: 'UserAllergies.label', default: 'UserAllergies'),
				id
			])
	...
	def update(Long id, Long version) {
		...
		if (!UserAllergiesInstance.save(flush: true)) {
			flash.message = message(code: 'Error updating the entry. Please ensure the values are correct.', args: [
				message(code: 'UserAllergies.label', default: 'UserAllergies'),
				id
			])
		...
		flash.message = message(code: 'Allergy \"' + UserAllergiesInstance.name + '\" updated successfully', args: [
			message(code: 'UserAllergies.label', default: 'UserAllergies'),
			UserAllergiesInstance.id
		])

		flash.tab = "allergies"
		redirect(controller: "conditions", action: "conditions", id: UserAllergiesInstance.id)
	...
	def delete(Long id) {
		...
		if (!UserAllergiesInstance) {
			...
			redirect(controller: "conditions", action: "conditions", tab: tab)
			return
		}

		try {
			UserAllergiesInstance.delete(flush: true)
			flash.message = message(code: 'Allergy \"' + UserAllergiesInstance.name + '\" deleted successfully', args: [
				message(code: 'UserAllergies.label', default: 'UserAllergies'),
				UserAllergiesInstance.id
			])
