...
class Medications2Controller {
	...
	def save() {
		...
		if (!UserMedicationsInstance.save(flush: true)) {
			flash.message = message(code: 'Error saving the entry. Please ensure the values are correct.', args: [
				message(code: 'UserMedications.label', default: 'UserMedications')
			])
			redirect(action: "create", params: params)
			return
		}

		flash.message = message(code: 'Medication \"' + UserMedicationsInstance.name + '\" added successfully', args: [
			message(code: 'UserMedications.label', default: 'UserMedications'),
			UserMedicationsInstance.id
		])
	...
	def update(Long id, Long version) {
		...
		flash.message = message(code: 'Medication \"' + UserMedicationsInstance.name + '\" updated successfully', args: [
			message(code: 'UserMedications.label', default: 'UserMedications'),
			UserMedicationsInstance.id
		])
	...
	def delete(Long id) {
		...
		try {
			...
			flash.message = message(code: 'Medicine \"' + UserMedicationsInstance.name + '\" deleted successfully', args: [
				message(code: 'UserMedications.label', default: 'UserMedications'),
				UserMedicationsInstance.id
			])
			redirect(controller: "healthInformation", action: "information")
