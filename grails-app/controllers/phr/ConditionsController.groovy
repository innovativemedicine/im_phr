...
class ConditionsController {
	...
	def save() {
		...
		if (!UserConditionsInstance.save(flush: true)) {
			flash.message = message(code: 'Error saving the entry. Please ensure the values are correct.', args: [
				message(code: 'UserConditions.label', default: 'UserConditions')
			])
			redirect(action: "create", params: params)
			return
		}

		flash.message = message(code: 'Condition \"' + UserConditionsInstance.name + '\" added successfully', args: [
			message(code: 'UserConditions.label', default: 'UserConditions'),
			UserConditionsInstance.id
		])
		redirect(action: "conditions", params: params)
	}
	...
	def update(Long id, Long version) {
		...
		flash.message = message(code: 'Condition \"' + UserConditionsInstance.name + '\" updated successfully', args: [
			message(code: 'UserConditions.label', default: 'UserConditions'),
			UserConditionsInstance.id
		])
	...
	def delete(Long id) {
		...
		try {
			...
			flash.message = message(code: 'Condition \"' + UserConditionsInstance.name + '\" deleted successfully', args: [
				message(code: 'UserConditions.label', default: 'UserConditions'),
				UserConditionsInstance.id
			])
			redirect(action: "conditions")
