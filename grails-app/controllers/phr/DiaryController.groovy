...
class DiaryController {
	...
	def save() {
		...
		if (!UserDiaryInstance.save(flush: true)) {
			flash.message = message(code: 'There was an error saving the diary entry.', args: [
				message(code: 'UserDiary.label', default: 'UserDiary')
			])
			render(view: "create", model: [UserDiaryInstance: UserDiaryInstance])
			return
		}

		flash.message = message(code: 'Diary entry added successfully', args: [
			message(code: 'UserDiary.label', default: 'UserDiary'),
			UserDiaryInstance.id
		])
		redirect(action: "diary", params: params)
	}
